...
from spider.task.tasks_seminually.seminually_mapping import doctor_seminually_crawl
...
def doctor_seminually_process():
    crawl_threads = Process(target=doctor_seminually_main)
    crawl_threads.start()

def doctor_seminually_main():
    ...
    saver_thread.join()
    end_time = time.time()
    logger.info("doctor_seminually task 用时：{}".format(end_time-start_time))
    logger.info("exit")
