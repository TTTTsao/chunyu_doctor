...
# weekly don't need too many threads
...
def task(task_name, e_id):
    ...
    if task_name == "hospital":
        crawl_hospital_clinic_enter_doctor(e_id)
...
def weekly_main():
    ...
    saver_thread.join()
    end_time = time.time()
    logger.info("weekly task(question, reward, enter_doctor) 用时：{}".format(end_time-start_time))
    logger.info("exit")
