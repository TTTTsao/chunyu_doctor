...
from spider.task.tasks_daily.daily_mapping import daily_crawl
...
def doctor_daily_process():
    crawl_threads = Process(target=doctor_daily_main)
    crawl_threads.start()

def doctor_daily_main():
    ...
    saver_thread.join()
    end_time = time.time()
    logger.info("doctor_daily task 用时：{}".format(end_time-start_time))
    logger.info("exit")
